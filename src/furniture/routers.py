...
from .services import FurnitureService, ImageService, CategoryService
from .validators import validate_media_type
from ..common.security import check_token
...
async def create_furniture(
        created_furniture: Annotated[schemas.CreateFurniture, Depends()],
    ...
    return Manager(
        CategoryService(session), FurnitureService(session), ImageService(session)
    ).create(created_furniture=created_furniture, files=files)


@router.put('/furniture/{furniture_id}')
async def update_furniture(
        furniture_id: Annotated[int, Path()],
        updated_furniture: Annotated[schemas.UpdateFurniture, Depends()],
        files: Annotated[list[UploadFile], Depends(validate_media_type)],
        session: Annotated[Session, Depends(session)]
):
    ...
    return Manager(
        CategoryService(session), FurnitureService(session), ImageService(session)
    ).update(furniture_id=furniture_id, updated_furniture=updated_furniture, files=files)


@router.put('/category/{category_id}')  # TODO доделать роутер
